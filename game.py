...
class GameApp:
  ...
  def __init__(self):
    ...
    self.levelFont = pygame.font.Font(self.resolve('img/msyhl.ttc'), 20)
...
def my_level(user):
  ...
  if user:
      import os
      if os.path.exists(path+"关卡.txt"):
          with open(path+"关卡.txt","r",encoding="UTF-8") as file:
            b = int(eval(file.read())[user])-1
